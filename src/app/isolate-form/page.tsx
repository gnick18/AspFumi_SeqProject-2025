    ...
    setFormData(prev => {
        ...
        if (field.startsWith('genotype.')) {
            ...
            if (key === 'present' && value !== 'Yes') {
                Object.keys(mutationState).forEach(k => { if (k !== 'present') mutationState[k] = ''; });
            }
            if (key === 'method') {
                (newState.genotype[mutation as keyof GenotypeInfo] as MutationInfo).markerGene = '';
                (newState.genotype[mutation as keyof GenotypeInfo] as MutationInfo).chemicalName = '';
                (newState.genotype[mutation as keyof GenotypeInfo] as MutationInfo).otherMethod = '';
            }
        } 
            ...
            geneState[key] = value;
            if (key === 'method') {
                (newState.otherGenes[index] as MutationInfo).markerGene = '';
                (newState.otherGenes[index] as MutationInfo).chemicalName = '';
                (newState.otherGenes[index] as MutationInfo).otherMethod = '';
            }
        }
        else {
    ...
    return (
        <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-white/50 p-8 rounded-lg shadow-md">
                <div className="mb-6"><h3 className="text-xl font-semibold mb-4" style={{ color: 'var(--slate-gray)' }}>Access Required</h3><p className="text-base mb-4">Please enter the password to access the submission form for isolate information. Each strain you send should have its own submission.</p></div>
                <form onSubmit={handlePasswordSubmit} className="space-y-4">
                    <div><label htmlFor="password" className="block text-sm font-medium mb-2">Password:</label><input type="password" id="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full p-3 border-2 rounded-lg" style={{ borderColor: 'var(--silver)' }} placeholder="Enter your password" required /></div>
